...
export default class extends think.controller.rest {
    ...
    async getAction() {
        ...
        let values = this.get();
        let where = {};
        if(values.title)  where.title =   ['like', '%' + values.title + '%'];
        data = await this.modelInstance.page(values.pageNo || 1, values.pageSize || 10).where(where).order('id DESC').countSelect();
        return this.success(data);
    }
